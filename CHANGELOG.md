...
- **SSH key support**: Parse SSH keys (`private-key`, `public-key`, `fingerprint`) from cipher text and vault item attachments.
...
- **Dashboard E2E test suite**: Full Playwright E2E test suite running in CI, covering secrets sync, SSH keys, attachments, stringData, context filtering, and dashboard views.
...
- CI/CD pipeline overhauled: simplified workflow triggers (pull_request only), Docker publish updates, arm64 builds removed from CI to avoid QEMU timeouts.
